...
describe('hooks', async () => {
    ...
    describe('#TolTipCheck', async () => {
        it('check tooltip on Iran with year 2000 and download map and csv', async () => {
            ...
            await driver.findElement(webdriver.By.id('exportLineBtn')).click()
  ...
  describe('#testFilterSliders', async () =>{
    it('Check that filtering and submit works', async () =>{
      ...
      console.log('vals',await mortalityLower.getText(),await incidenceLower.getText(),await deathPercentageLower.getText());

      assert.equalTrue(parseFloat(await mortalityLower.getText()) > 0);
      assert.equalTrue(parseFloat(await incidenceLower.getText()) > 0);
      assert.equalTrue(parseFloat(await deathPercentageLower.getText()) > 0);
